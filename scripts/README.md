...
**Fork of https://github.com/heptiolabs/checkr**

checkr is a library for implementing Kubernetes [liveness and readiness](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/) probe handlers in your Go application.
...
## Usage
 ...
 - Create a `checkr.Handler`:
   ```go
   health := checkr.NewHandler()
   ```

 - Configure some application-specific liveness checks (whether the app itself is unhealthy):
   ```go
   // Our app is not happy if we've got more than 100 goroutines running.
   health.AddLivenessCheck("goroutine-threshold", checkr.GoroutineCountCheck(100))
   ```
 ...
 - Configure some application-specific readiness checks (whether the app is ready to serve requests):
   ...
   health.AddReadinessCheck(
       "upstream-dep-dns",
       checkr.DNSResolveCheck("upstream.example.com", 50*time.Millisecond))

   // Our app is not ready if we can't connect to our database (`var db *sql.DB`) in <1s.
   health.AddReadinessCheck("database", checkr.DatabasePingCheck(db, 1*time.Second))
   ```
